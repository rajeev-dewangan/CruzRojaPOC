...
${context}`;
    ...
    // Thinking models can return content as an array of parts rather than a
    // plain string, so pull out the text parts instead of blind-casting.
    const reply = (typeof res.content === "string"
      ? res.content
      : (res.content ?? [])
    ...
    // A MAX_TOKENS finish means the answer was cut off mid-sentence. Escalate
    // rather than sending the user half a reply.
    if (!reply || res.response_metadata?.finishReason === "MAX_TOKENS") {
      console.error(
        `[chat] unusable reply finish=${res.response_metadata?.finishReason} len=${reply.length}`
      );
      const fallback =
      ...
      await supabaseAdmin.from("messages").insert({
        conversation_id: convId, role: "assistant", content: fallback,
        similarity_score: topScore, escalated: true,
      });
      await supabaseAdmin.from("conversations")
        .update({ status: "escalated" }).eq("id", convId);
      ...
      return NextResponse.json(
        { reply: fallback, escalated: true, score: topScore, conversationId: convId },
        { headers }
      );
    }
